...
echo 'python -m src.scripts.preprocess_dataset dataset=videodc_dl3dv dataset.data_root='"${root}"' stage='"${stage}"' dataset.subset='"${subset}"' output_dir='"${output_dir}"' flip='"${flip}"' index="${SLURM_ARRAY_TASK_ID}" size='"${size}"' dataset.num_workers='"${CPUS}">> $jobscript
