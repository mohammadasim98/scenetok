...
echo "srun --mem-per-gpu=${MEMORY} python -m src.main_scene +experiment=${config} data_loader.train.num_workers=${CPUS} hydra.run.dir=./outputs/${config}/${id} mode=train hydra.job.name=train trainer.task_steps=${TASK_STEPS} trainer.devices=${GPUS} trainer.num_nodes=${NUM_NODES} &" >> $jobscript
echo "wait" >> $jobscript
